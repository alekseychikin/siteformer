...
import configsTemplate from './configs-radio.gutt'

export default class ConfigsRadio extends Modal {
	get events() {
		return {
			'input change: [data-role="configs-radio-option-label"]': this.changeOptionValueHandler,
			'click: [data-role="option-remove"]': this.clickRemoveOptionHandler,
			'click: [data-role="add-value"]': this.clickAddOptionHandler,
			'change: [data-role="configs-radio-option"]': this.changeOptionCheckedHandler,
