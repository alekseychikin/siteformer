...
},{"asap/raw":9}],2:[function(require,module,exports){
'use strict';
...
},{"./core.js":1,"asap":8}],7:[function(require,module,exports){
'use strict';
...
},{"./core.js":1}],8:[function(require,module,exports){
"use strict";
...
},{}],"jquery-plugins.coffee":[function(require,module,exports){
var jQuery;
...
require("jquery/jquery.role");

require("jquery/jquery.modifiers");

require("jquery/jquery.utils");
...
ViewPrototype = {
  ...
  destroy: function() {
    ...
    if (this.events && this.contain) {
      ref = this.cachedEvents;
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        item = ref[i];
        if (item.selector) {
          results.push(this.contain.off(item.event, item.selector, item.callback));
        } else if (item.target) {
          results.push(item.target.off(item.event, item.callback));
        } else {
          results.push(void 0);
        }
      }
  ...
  generateEvent: function(event, selector, cb, target) {
    var self;
    if (target == null) {
      target = null;
    }
    ...
    return function() {
      if (self.debug) {
        console.log("Event `" + event + "` at `" + selector + "`");
      }
      if (target) {
  ...
  cacheEvent: function(event, selector, cb, target) {
    var callback, element;
    if (target == null) {
      target = null;
    }
    callback = this.generateEvent(event, selector, cb, target);
    element = {
      event: event,
      callback: callback
    };
    ...
    this.cachedEvents.push(element);
    return callback;
  },
  addEvent: function(selector, event, cb) {
    var callback, target;
    target = null;
    switch (selector) {
      case "window":
    ...
    if (target) {
      if (this.debug) {
        console.log("Add event `" + event + "` at `" + selector + "`");
      }
      callback = this.cacheEvent(event, selector, cb, target);
      return $(target).on(event, callback);
    } else {
      if (this.debug) {
        console.log("Add event `" + event + "` at `" + selector + "` (" + (this.contain.find(selector)).length + " selectors)");
      }
      callback = this.cacheEvent(event, selector, cb);
      return this.contain.on(event, selector, callback);
    }
  },
  __initial: function(params) {
    var cb, event, fn, ref, self;
    if (params == null) {
      params = null;
    }
    self = this;
    if (this.events && !this.contain && this.debug) {
      console.error("There are events without contain");
    }
    if (this.events && this.contain) {
      ref = this.events;
      fn = (function(_this) {
        return function(event, cb) {
          ...
          if (typeof cb !== "function" && !_this[cb]) {
            console.error(cb + " does not exists at", _this);
            cb = null;
          }
          if (typeof cb !== "function" && typeof _this[cb] === "function") {
            cb = _this[cb];
          }
          if (cb) {
            return _this.addEvent(selector, event, cb);
          }
        };
      })(this);
      for (event in ref) {
    ...
    if (this.initial && !this.contain && this.debug) {
      console.warn("There is initial without contain");
    }
    if (this.initial && this.contain) {
      return this.initial.apply(this, params);
    }
  },
  on: function(eventName, callback) {
    if (this.extEvents[eventName] == null) {
      this.extEvents[eventName] = [];
    }
    return this.extEvents[eventName].push(callback);
  },
  trigger: function() {
    var event, eventName, i, len, ref, results, values;
    eventName = arguments[0], values = 2 <= arguments.length ? slice.call(arguments, 1) : [];
    if (this.extEvents[eventName]) {
      ref = this.extEvents[eventName];
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        event = ref[i];
        results.push(event.apply(this, values));
      }
      return results;
    }
  },
...
ViewItem = function(params, target, model) {
  var field, fn, item, self;
  if (!(this instanceof ViewItem)) {
    return (function(func, args, ctor) {
      ctor.prototype = func.prototype;
      var child = new ctor, result = func.apply(child, args);
      return Object(result) === result ? result : child;
    })(ViewItem, arguments, function(){});
  }
  this.cachedEvents = [];
  this.contain = target;
  this.model = model;
  ...
  if (this.model) {
    ...
    this.model.triggerInitialTriggers();
  }
  return this;
};
...
},{"jquery-plugins.coffee":"jquery-plugins.coffee"}]},{},["view.coffee"]);

require=(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({"model.coffee":[function(require,module,exports){
var Model, ModelItem, ModelPrototype, getElement, timeout,
  slice = [].slice;

getElement = function(root, params) {
  var elementPath, i, key, value;
  i = 0;
  value = params;
  elementPath = null;
  ...
  return [root, value, elementPath];
};

timeout = {};

ModelPrototype = {
  clone: function(obj, cachedElements) {
    ...
    if ((cachedElements.indexOf(obj)) === -1) {
      ...
      if (obj instanceof Array) {
        ...
        for (i = j = 0, len = obj.length; j < len; i = ++j) {
          elem = obj[i];
          copy[i] = this.clone(elem, cachedElements);
        }
        return copy;
      }
      if (obj instanceof Object) {
        ...
        for (i in obj) {
          attr = obj[i];
          copy[i] = this.clone(attr, cachedElements);
        }
        return copy;
      }
    } else {
  ...
  triggerInitialTriggers: function() {
    var event, j, k, len, len1, params, ref, ref1, results, value;
    ref = this.initialTriggers;
    results = [];
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      results.push(this.trigger.apply(this, params));
    }
    return results;
  },
  trigger: function() {
    var event, eventName, j, k, len, len1, ref, ref1, results, values, view;
    eventName = arguments[0], values = 2 <= arguments.length ? slice.call(arguments, 1) : [];
    if (this.doRecordInitialTriggers) {
      return this.initialTriggers.push({
        eventName: eventName,
        values: values
      });
    } else {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        if (typeof view.render === "function") {
          view.render.call(view, this.state);
        }
      }
      if (this.events[eventName]) {
        ref1 = this.events[eventName];
        results = [];
        for (k = 0, len1 = ref1.length; k < len1; k++) {
          event = ref1[k];
          results.push(event.apply(null, values));
        }
        return results;
      }
    }
  ...
  addRenderListener: function(view) {
    this.views.push(view);
    if (this.initializedState) {
      return this.trigger("initialState", this.state);
    }
  },
  triggerUpdate: function(elementPath) {
    if (timeout[elementPath]) {
      clearTimeout(timeout[elementPath]);
      delete timeout[elementPath];
    }
    return timeout[elementPath] = setTimeout((function(_this) {
      return function() {
        return _this.trigger(elementPath, _this.state[elementPath]);
      };
    })(this), 1);
  },
...
ModelItem = function(params, state) {
  var field, fn, item, result;
  if (!(this instanceof ModelItem)) {
    return (function(func, args, ctor) {
      ctor.prototype = func.prototype;
      var child = new ctor, result = func.apply(child, args);
      return Object(result) === result ? result : child;
    })(ModelItem, arguments, function(){});
  }
  this.doRecordInitialTriggers = false;
  ...
  if (this.initial && ((this.autoInit == null) || this.autoInit === true)) {
    this.doRecordInitialTriggers = true;
...
require=(function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({"render":[function(require,module,exports){
(function (factory)
{
...
createXMLHTTPObject = function() {
  var XMLHttpFactories, e, error, i, j, ref, xmlhttp;
  XMLHttpFactories = [
    function() {
...
httpFile = function(url, data) {
  return new Promise(function(resolve, reject) {
    var field, file, formData, i, input, j, len, ref, req;
    if (window.FormData) {
...
parsePostData = function(postData, name) {
  var attr, data, i, j, len, ref, str;
  if (name == null) {
    name = "";
  }
  str = '';
  if ((ref = typeof postData) === "string" || ref === "number" || ref === "boolean") {
    ...
    for (i = j = 0, len = postData.length; j < len; i = ++j) {
      attr = postData[i];
      data.push(parsePostData(attr, (name.length ? name + "[" + i + "]" : i)));
    }
    return data.join("&");
    ...
    for (i in postData) {
      attr = postData[i];
      if (!(Object.prototype.hasOwnProperty.call(postData, i))) {
        continue;
      }
      data.push(parsePostData(attr, (name.length ? name + "[" + i + "]" : i)));
    }
...
},{"components/form/form__date.coffee":1,"components/form/form__select.coffee":2,"components/form/form__time.coffee":3,"jquery-plugins.coffee":"jquery-plugins.coffee"}],5:[function(require,module,exports){
require("components/form/form");


},{"components/form/form":4}]},{},[5]);
