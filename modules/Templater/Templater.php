...
class SFTemplater
{
  ...
  private static function isNeedToCompile(
    $templatePath,
    $compileTimes,
    $templates,
    $compiles
  ...
  private static function recursiveTemplates($path, $templates = []) {
    $dir = opendir($path);
  ...
  public static function requireJS($path, $attrs = '') {
    self::$scripts[] = ['path' => $path, 'attrs' => $attrs];
  }
  ...
  public static function inc($templatePath, $params = []) {
    ...
    foreach ($variables_ as $name => $value) {
      $variables .= 'var ' . $name . ' = ' . json_encode($value, true) . ';' . N;
    }
